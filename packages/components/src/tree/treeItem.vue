...
const recursiveFunction = function (items) {
  let str = ''
  ...
  if (props.multiple) {
    if (props.defaultSelectNodes && props.defaultSelectNodes.length) {
      props.defaultSelectNodes.forEach((v, i) => {
        ...
        const nodeItems = getParents(props.options, v)
        getSameLevelChile(nodeItems[1], nodeItems[nodeItems.length - 1])
      })
    }
  }
...
<template>
  <div class="mzl-tree-item-box">
    <li>
      ...
      <div
        class="mzl-tree-lable-box"
        :class="[
          {
            'label-active': tabIndexs === items.key,
            'mzl-tree-lable-disabled': items.disabled,
          },
        ]"
        :data-key="dataKey"
        :style="{ 'padding-left': `${(index as number) * 15}px` }"
        @click.stop.prevent="toggle(items)"
      >
        <span
          class="mzl-tree-lable-span"
          :class="[
            {
              rotate:
        ...
        <span
          v-if="multiple"
          class="mzl-tree-label-select-span"
          :class="[
            {
              'mzl-tree-label-select-span-active': items.isSelected,
              'mzl-tree-label-select-span-active-1': items.semiSelected,
            },
          ]"
          @click.stop="selectClick(items, index)"
        />
